 ...
 *
 * A textarea holds one colour, and a condition is SQL like any other: it deserves the colours the
 * language server's stream resolved in it. So the text is drawn twice — painted here, and typed in
 * a transparent field over it — which is why every metric that decides where a character lands is
 * shared between the two in the stylesheet rather than set on each.
 *
...
export function FilterHighlight({
  ...
  const painted = useMemo(
    () => withSemanticTokens(plainPostgresSource(postgresSourceLines(text)), named),
    [text, named],
  );
