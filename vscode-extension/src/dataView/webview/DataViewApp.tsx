...
import type { GridEditing } from "../../sqlNotebookRenderer/CellEditor.js";
import { type GridLayout, ResultGrid } from "../../sqlNotebookRenderer/ResultGrid.js";
import { resultAsTsv } from "../../sqlNotebookRenderer/resultFormatting.js";
import { resultRowSummary } from "../../sqlNotebookRenderer/SqlResultView.js";
import { hasWorkbenchTreeDrag } from "../../workbenchGraph/dragAndDrop.js";
import type {
  DataViewAddition,
  DataViewCompletion,
  DataViewEdit,
...
import { dataViewColumnKeys, dataViewSourceTitle, sameDataViewRow } from "../protocol.js";
...
export function DataViewApp({ messaging }: { messaging: DataViewMessaging }) {
  ...
  const editing = useMemo<GridEditing | undefined>(() => {
    ...
    return {
      ...
      editFor(row, _rowIndex, ordinal) {
        ...
        return state.edits.find((edit) => edit.ordinal === ordinal && sameDataViewRow(edit, probe));
      },
  ...
  const requestSort = (ordinal: number, additive: boolean) => {
    ...
    if (!column) return;
    const current = columnSorts.find((sort) => sort.column === column);
    const next =
      current === undefined
        ? ({ column, direction: "ascending" } as const)
        : current.direction === "ascending"
          ? ({ column, direction: "descending" } as const)
          : undefined;
    const others = additive ? columnSorts.filter((sort) => sort.column !== column) : [];
    applySorts(next ? [...others, next] : others);
  };
            ...
            <>
              ...
              <div className="column-menu toolbar-menu" role="menu">
                ...
                {(["csv", "tsv", "json"] as const).map((format) => (
                  <MenuItem
                    key={`loaded-${format}`}
                    label={`Export loaded rows as ${format.toUpperCase()}…`}
                    disabled={!payload || payload.columns.length === 0}
                    onSelect={() => {
                      setMoreOpen(false);
                      post({ type: "data-view/export", format, scope: "loaded" });
                    }}
                  />
                ))}
                {(["csv", "tsv", "json"] as const).map((format) => (
                  <MenuItem
                    key={`all-${format}`}
                    label={`Export all rows as ${format.toUpperCase()}…`}
                    disabled={!payload || payload.columns.length === 0}
                    onSelect={() => {
                      setMoreOpen(false);
                      post({ type: "data-view/export", format, scope: "all" });
                    }}
                  />
                ))}
                <MenuItem
