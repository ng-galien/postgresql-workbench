...
describe("JOIN composition through the planner", () => {
  ...
  it("joins through the mapping table and projects only the target's columns", () => {
  ...
  it("keeps WHERE and ORDER BY intact when a JOIN is inserted before them", () => {
    const result = composePostgresSql(
      request(
        "SELECT\n  p.id\nFROM\n  shop.product AS p\nWHERE\n  p.id > 1\nORDER BY\n  p.id DESC",
        2,
  ...
  it("offers every shortest path when several keys lead to the target, then applies the choice", () => {
    const first = composePostgresSql(
      request("SELECT o.id FROM shop.sales_order AS o", 5, "address"),
      snapshot,
    );
    ...
    const chosen = composePostgresSql(
